 ...
 *
 * This class isn't meant to be directly used. Instead you define validators
 * thru static variables in your Model.
...
class Validations
{
    ...
    public function __construct(Model $model)
    {
        ...
        $this->validators = array_intersect(
            array_keys($this->class->getStaticProperties()),
    ...
    public function rules()
    {
        ...
        foreach ($this->validators as $validate)
        {
            ...
            foreach (String::wrapIntoArray($attrs) as $attr)
            {
                $field = $attr[0];
    ...
    public function validate()
    {
        foreach ($this->validators as $validate)
        {
            ...
            $this->$validate(String::wrapIntoArray($definition));
        }
     ...
     *
     * <code>
     * class Car extends Emyi\Db\Model {
    ...
    public function validates_length_of($attrs)
    {
        ...
        foreach ($attrs as $attr)
        {
            ...
            if ($range_options[0] == 'within' || $range_options[0] == 'in') {
                ...
                if (!is_array($range) || (int) $range[0] >= (int) $range[1])
                    throw new  ValidationsArgumentError(
                        "$range_option must be an array composing a range of
                        numbers with key [0] being less than key [1]"
                    );
    ...
    public function validates_uniqueness_of($attrs)
    {
        ...
        foreach ($attrs as $attr)
        {
            ...
            $primary_key_value = $this->model->$primary_key[0];
            ...
            $sql = "";
            $conditions = array("");
            $primary_key_quoted = $connection->quoteName($primary_key[0]);
            if ($primary_key_value === null)
                $sql = "{$primary_key_quoted} IS NOT NULL";
            else
            {
                $sql = "{$primary_key_quoted} != ?";
                array_push($conditions, $primary_key_value);
            }
    ...
    private function is_blank_with_option($var, &$options)
    {
        return (
            '' === trim($var) &&
            (isset($options['allow_blank']) && $options['allow_blank'])
        );
    }
}
