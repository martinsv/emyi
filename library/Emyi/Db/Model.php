...
{
    /**
     * An instance of Errors and will be instantiated once a write method is
     * called.
     * @var Errors
     */
    public $errors;
     ...
     * it will either be null or an array of column_names that have been
     * modified
     * @var array
     */
    ...
    /**
     * Flag that determines if a call to save() should issue an insert or an
     ...
     * @var array
     */
    protected static $VALID_OPTIONS = [
        'conditions',
        'limit',
        'offset',
        'order',
    ...
    /**
     * Database's datetime format specif to this Model
     * @var string
     */
    public $datetime_format = 'Y-m-d H:i:s T';
    ...
    /**
     * Set this to explicitly specify the model's table name if different
     * from inferred name.
     *
     * If your table doesn't follow our table name convention you can set this
     * to the name of your table to explicitly tell ActiveRecord what your
     * table is called.
     *
     * @var string
     */
    ...
    /**
     * Set this to override the default primary key name if different from
     * default name of "id".
     * @var string
     */
     ...
     * such as constructing a model or using update_attributes.
     *
     ...
     *
     * This is the opposite of $attr_accessible and the format
     * for defining these are exactly the same.
     *
     * If the attribute is both accessible and protected, it is treated as
     * protected.
     *
     * @var array
     */
     ...
     * Constructs a model.
     *
     * When a user instantiates a new object (e.g.: it was not the ORM that
     * instantiated via a find) then $attributes will be mapped according to
     * the schema's defaults. Otherwise, the given $attributes is be mapped
     * via set_attributes_via_mass_assignment.
     *
     * <code>
     * new Person(array('first_name' => 'Tito', 'last_name' => 'the Grief'));
     * </code>
     *
     * @param array $attributes Hash containing names and values to mass assign
     *      to the model
     * @param boolean $guard_attributes Set to true to guard
     *      protected/non-accessible attributes
     * @param boolean $instantiating_via_find Set to true if this model is
     *      being created from a find call
     * @param boolean $new_record Set to true if this should be considered a
     *      new record
     * @return Emyi\Db\Model
     */
    public function __construct(
        ...
        // since all attribute assignment now goes thru assign_attributes()
        // we want to reset dirty if instantiating via find since nothing is
        // really dirty when doing that
        if ($instantiating_via_find) {
            $this->is_dirty = [];
        }
    ...
    /**
     * Magic method which delegates to read_attribute(). This handles firing
     * off getter methods, as they are not checked/invoked inside of
     * read_attribute(). This circumvents the problem with a getter being
     * accessed with the same name as an actual attribute.
     *
     * You can also define customer getter methods for the model.
     *
     * EXAMPLE:
     * <code>
     * class User extends Emyi\Db\Model {
     ...
     *
     * $user = new User();
     * // will call $user->get_middle_name()
     ...
     * @throws UnexpectedValueException if $name does not exist
     */
    public function __set($name, $value)
    {
        ...
        throw new UnexpectedValueException(get_called_class(), $name);
    }
     ...
     * model's primary key no matter what the actual attribute name is
     * for the primary key.
     *
     * @param string $name Name of an attribute
     * @return mixed The value of the attribute
     * @throws UnexpectedValueException if name could not be resolved to an
    ...
    public function &read_attribute($name)
    {
        ...
        if ($name == 'id') {
            ...
            if (isset($this->attributes[$primary_key])) {
                return $this->attributes[$primary_key];
            }
        }
    ...
    public function dirty_attributes()
    {
        ...
        $dirty = array_intersect_key($this->attributes, $this->is_dirty);
     ...
     *
     * @param boolean Set to true to return the first value in the primary_key
     *      array only
     * @return string The primary key for the model
     */
    public function get_primary_key($first = false)
    {
        $primary_key = static::table()->primary_key;
        return $first ? $primary_key[0] : $primary_key;
    }
    ...
    public function get_validation_rules()
    {
        ...
        return (new Validations($this))->rules();
    }

    /**
     * Returns an associative array containing values for all the attributes
     * in $attributes
     *
     * @param array array containing attribute names
     ...
     *
     * @param string $method_name name of method that was invoked on model for
     *      exception message
     * @throws Emyi\Db\ReadOnlyException
     */
    ...
    /**
     * Returns the Emyi\Db\Table object for this model.
     * Be sure to call in static scoping: static::table()
     *
     * @return Emyi\Db\Table
     */
     ...
     *
     * This function will automatically determine if an INSERT or UPDATE needs
     * to occur. If a validation or a callback for this model returns false,
     * then the model will not be saved and this will return false.
     *
     * If saving an existing model only data that has changed will be saved.
     *
     * @param boolean $validate Set to true or false depending on if you want
     *      the validators to run or not
     * @return boolean true if the model was saved to the database otherwise
     *      false
     */
    ...
    private function insert($validate=true)
    {
        ...
        if ($table->sequence && !isset($attributes[$primary_key])) {
            if (($conn = static::connection()) instanceof Adapter\OciAdapter) {
                // terrible oracle makes us select the nextval first
                $attributes[$primary_key] = $conn->get_nextSequenceValue($table->sequence);
                $table->insert($attributes);
                $this->attributes[$primary_key] = $attributes[$primary_key];
            } else {
                // unset primary_key that was set to null
                if (array_key_exists($primary_key, $attributes)) {
                    unset($attributes[$primary_key]);
                }

                $table->insert($attributes, $primary_key, $table->sequence);
                $use_sequence = true;
            }
        } else {
            $table->insert($attributes);
        ...
        // if we've got an autoincrementing/sequenced primary_key set, it
        // don't need this check until the day comes that we decide to support
        // composite primary_keys
        // if (count($primary_key) == 1)
        {
            $column = $table->get_column_by_inflected_name($primary_key);

            if ($column->auto_increment || $use_sequence) {
                $this->attributes[$primary_key] = static::connection()->lastInsertId($table->sequence);
            }
        }
    ...
    private function update($validate=true)
    {
        ...
        if ($this->isDirty()) {
            $primary_key = $this->values_for_primary_key();

            if (empty($primary_key)) {
                throw new Exception(
                    "Cannot update, no primary key defined for: " . get_called_class()
                );
            }
            ...
            static::table()->update($dirty, $primary_key);
    ...
    public static function delete_all($options=[])
    {
        ...
        $sql = new SQLBuilder($conn, $table->getQualifiedTableName());

        $conditions = is_array($options) ? $options['conditions'] : $options;

        if (is_array($conditions) && !ArrayHelper::is_hash($conditions)) {
    ...
    public function delete()
    {
        ...
        if (empty($primary_key)) {
            throw new Exception("Cannot delete, no primary key defined for: " . get_called_class());
        }
        ...
        static::table()->delete($primary_key);
     ...
     */
    public function values_for_primary_key()
    {
        return $this->values_for(static::table()->primary_key);
    }
    ...
    {
        return empty($this->is_dirty) ? false : true;
    }
     ...
     *
     * @throws Emyi\Db\UnexpectedValueException
     ...
     */
    private function setAttributesViaMassAssignment(array &$attributes, $guard_attributes)
    {
        ...
        foreach ($attributes as $name => $value)
        {
            ...
            if (array_key_exists($name, $table->columns)) {
                $old_datetime_format = $connection::$datetime_format;
                $connection::$datetime_format = $this->datetime_format;
                $value = $table->columns[$name]->cast($value, $connection);
                $name = $table->columns[$name]->inflected_name;
                $connection::$datetime_format = $old_datetime_format;
            }
        ...
        if (!empty($exceptions)) {
            ...
            throw new UnexpectedValueException(get_called_class(), $exceptions);
        }
    }
    ...
    public function set_relationship_from_eager_load(Model $model=null, $name)
    {
        ...
        throw new RelationshipException(
            "Relationship named $name has not been declared
             for class: {$table->class->getName()}"
        );
    }
    ...
    {
        $this->relationships = [];
        $primary_key = array_values($this->get_values_for($this->get_primary_key()));

        $this->setAttributesViaMassAssignment($this->find($primary_key)->attributes, false);
        $this->resetDirty();
    ...
    public static function __callStatic($method, $args)
    {
        ...
        if (substr($method, 0, 17) == 'find_or_create_by') {
            ...
            if (strpos($attributes, '_or_') !== false) {
                throw new Exception("Cannot use OR'd attributes in find_or_create_by");
            }
        ...
        if (substr($method, 0, 7) === 'find_by') {
            ...
            $options['conditions'] =
                SQLBuilder::create_conditions_from_underscored_string(
                    static::connection(),
                    $attributes,
                    $args,
            ...
            if (!($ret = static::find('first', $options)) && $create) {
                return static::create(
                    SQLBuilder::create_hash_from_underscored_string(
                        $attributes,
                        $args,
                        static::$alias_attribute
                    ));
    ...
    public function __call($method, $args)
    {
        ...
        if (preg_match('/(build|create)_/', $method))
        {
            ...
            if (($association = $table->getRelationship($association_name)) ||
                ($association = $table->getRelationship($association_name = String::pluralize($association_name))))
            {
                // access association to ensure that the relationship has been
                // loaded so that we do not double-up on records if we append
                // a newly created
    ...
    public static function count(/* ... */)
    {
        ...
        {
            if (ArrayHelper::is_hash($args[0]))
                $options['conditions'] = $args[0];
            else
                $options['conditions'] = call_user_func_array('static::primary_key_conditions', $args);
        }

        $table = static::table();
        $sql = $table->optionsToSql($options);
        $values = $sql->getWhereValues();
        return static::connection()->fetchOne($sql->toString(), $values);
    }
     ...
     *
     * # finding by primary_key accepts an options array
     ...
     * </ul>
     *
     * @throws {@link RecordNotFound} if no options are passed or finding by primary_key and no records matched
    ...
    public static function find(/* $type, $options */)
    {
            ...
            switch ($args[0])
            {
                ...
                case 'last':
                    if (!array_key_exists('order', $options))
                        $options['order'] = join(' DESC, ', static::table()->primary_key) . ' DESC';
                    else
        ...
        // anything left in $args is a find by primary_key
        if ($num_args > 0 && !isset($options['conditions']))
            return static::find_by_primary_key($args, $options);
     ...
     *
     * @see find
     * @param array $values An array containing values for the primary_key
     * @param array $options An options array
     * @return Model
     * @throws {@link RecordNotFound} if a record could not be found
     */
    public static function find_by_primary_key($values, $options)
    {
        $options['conditions'] = static::primary_key_conditions($values);
        $list = static::table()->find($options);
        $results = count($list);
    ...
    {
        if (ArrayHelper::is_hash($array))
        {
            $keys = array_keys($array);
            $diff = array_diff($keys, static::$VALID_OPTIONS);

            if (!empty($diff) && $throw)
                throw new Exception("Unknown key(s): " . join(', ', $diff));

            $intersect = array_intersect($keys, static::$VALID_OPTIONS);
     ...
     * @return array An array in the form array(name => value, ...)
     */
    public static function primary_key_conditions($args)
    {
        $table = static::table();
        $ret = array($table->primary_key[0] => $args);
        return $ret;
    }
   ...
   * Ex:
   * <code>
   * Emyi\Db\CsvSerializer::$delimiter=';';
   * Emyi\Db\CsvSerializer::$enclosure='';
