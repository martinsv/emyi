...
class CallBack
{
     ...
     * Creates a CallBack.
     *
     * @param string $model_class_name The name of a Emyi\Db\Model class
     * @return Emyi\Db\CallBack
     */
    public function __construct($model_class_name)
    {
        $this->class = Reflections::instance()->get($model_class_name);
        ...
        foreach (static::$VALID_CALLBACKS as $name) {
            ...
            } elseif ($this->class->hasMethod($name)) {
     ...
     *
     * @param string name of a callback see Emyi\Db\CallBack::$VALID_CALLBACKS
    ...
    public function invoke($model, $name, $must_exist = true)
    {
        if ($must_exist && !array_key_exists($name, $this->registry))
            throw new Exception("No callbacks were defined for: $name on " . get_class($model));
     ...
     * @return void
     * @throws Exception if invalid callback type or callback method was not found
     */
    public function register($name, $closure_or_method_name=null, $options=[])
    {
        ...
        if (!in_array($name, self::$VALID_CALLBACKS))
            throw new Exception("Invalid callback: $name");

        if (!($closure_or_method_name instanceof Closure))
        {
            if (!isset($this->publicMethods))
                $this->publicMethods = get_class_methods($this->class->getName());

            if (!in_array($closure_or_method_name, $this->publicMethods))
            {
                if ($this->class->hasMethod($closure_or_method_name))
                {
                    // Method is private or protected
                    throw new Exception("Callback methods need to be public (or anonymous closures). " .
                        "Please change the visibility of " . $this->class->getName() . "->" . $closure_or_method_name . "()");
                }
                else
                {
                    // i'm a dirty ruby programmer
                    throw new Exception("Unknown method for callback: $name" .
