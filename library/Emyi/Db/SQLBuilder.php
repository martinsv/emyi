...
class SQLBuilder
{
     ...
     * @return SQLBuilder
     * @throws Exception if connection was invalid
     */
    ...
    public function bindValues()
    {
        ...
        return ArrayHelper::flatten($ret);
    }
    ...
    public function insert($hash, $primary_key=null, $sequence_name=null)
    {
        if (!ArrayHelper::is_hash($hash))
        ...
        if ($primary_key && $sequence_name)
            $this->sequence = array($primary_key, $sequence_name);
    ...
    private function apply_where_conditions($args)
    {
        ...
        if ($num_args == 1 && ArrayHelper::is_hash($args[0]))
        {
            ...
            $this->where_values = ArrayHelper::flatten($e->values());
        }
        elseif ($num_args > 0)
        {
